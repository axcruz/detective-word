...
const createWordSearch = (n, w) => {
  ...
  if (w.length > maxWords) {
    console.error(
      "Error: The total number of words must be at most " + maxWords + "."
    );
    return null;
  }
  ...
  function canPlaceWord(word, row, col, direction) {
    if (direction === "horizontal") {
      ...
      for (let i = 0; i < word.length; i++) {
        if (
          isPositionPlaced(row, col + i) &&
          wordSearchArray[row][col + i] !== word[i]
        ) {
          return false;
        }
      }
      ...
      for (let i = 0; i < word.length; i++) {
        if (
          isPositionPlaced(row + i, col) &&
          wordSearchArray[row + i][col] !== word[i]
        ) {
          return false;
        }
      }
      ...
      for (let i = 0; i < word.length; i++) {
        if (
          isPositionPlaced(row + i, col + i) &&
          wordSearchArray[row + i][col + i] !== word[i]
        ) {
          return false;
        }
      }
  ...
  function placeWord(word, row, col, direction) {
    const transformWord =
      Math.random() < 0.5 ? word : word.split("").reverse().join(""); // Randomly reverse the order of the word
  ...
  for (const word of w) {
    ...
    while (!placed) {
      ...
      const direction = ["horizontal", "vertical", "diagonal"][
